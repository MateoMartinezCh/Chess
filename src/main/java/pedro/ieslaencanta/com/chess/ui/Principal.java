...
public class Principal extends Application implements EventListener {
    ...
    public void start(Stage stage) throws Exception {
        ...
        stage.setTitle("DAW Ajedrez Mateo");
    ...
    private void seleccionarCelda(MouseEvent m) {
        ...
        if (element.getAttribute("class").equals("figura") && this.game.getPieceType(f, c) == this.game.getTurno()) {
    ...
    public void moverCelda(MouseEvent m) {
        ...
        this.msg = new Message("Mensaje pendiente", 1500, this.doc);
        ...
        if (this.punto_origen.getX() != this.punto_destino.getX() || this.punto_origen.getY() != this.punto_destino.getY()) {
            ...
            if (this.game.canMove((int) this.punto_origen.getY(), (int) this.punto_origen.getX(), (int) this.punto_destino.getY(), (int) this.punto_destino.getX())) {
                ...
                //HE CAMBIARDO CALCULATED MOVES POR MOVES
                mvs = this.game.getMoves((int) punto_origen.getY(), (int) punto_origen.getX());
                ...
                jaques = this.game.Jaque();
                //se produce un mate
                if (jaques != null && jaques.length > 0) {
    ...
    public void handleEvent(Event event) {
        String mensaje="";
        this.msgturno = new Message(mensaje, 500, this.doc);
        if (event instanceof MouseEvent) {
            ...
            if (this.game.getTurno() == PieceType.White) {
                mensaje="Turno de blancas";
                this.msgturno.setMsg(mensaje);
                this.msgturno.startTurno();
            } else {
                mensaje="Turno de negras";
                this.msgturno.setMsg(mensaje);
                this.msgturno.startTurno();
            }
        }
            ...
            for (int j = 0; j < b.getWidht(); j++) {
                if (b.getCell(i, j) != null && b.getCell(i, j).getPiece() != null
                    ...
                    if (tempo != null) {
                        ...
                        throw new UnsupportedOperationException("Te faltan piezas por pintar");
                    }
                    switch (name) {
                        case ("RW"):
